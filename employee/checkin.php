...
// --- 2. คำนวณสถานะ (สาย/ตรงเวลา) ---
    ...
    $status = 'on_time'; // ตั้งเป็นค่าเริ่มต้น
    $today_date = date('Y-m-d'); // วันที่ปัจจุบัน

    // (*** แก้ไข: ค้นหากะของวันนี้จากตาราง employee_schedules ***)
    $stmt_schedule = $conn->prepare(
        "SELECT s.time_in 
         FROM employee_schedules es
         JOIN schedule_types s ON es.schedule_id = s.id
         WHERE es.user_id = ? AND es.shift_date = ?"
    );
    $stmt_schedule->bind_param("is", $user_id, $today_date);
    $stmt_schedule->execute();
    $result_schedule = $stmt_schedule->get_result();

    if ($result_schedule->num_rows > 0) { // ถ้าวันนี้ "มีกะ"
        ...
        // ถ้าวันนี้ "ไม่มีกะ" แต่มาทำงาน
        // เราจะถือว่า "ตรงเวลา" (หรือคุณจะกำหนดสถานะอื่นก็ได้)
        $status = 'on_time';
    }
    $stmt_schedule->close();
    // --- จบส่วนแก้ไข ---
