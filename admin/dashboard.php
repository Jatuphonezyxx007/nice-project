...
$stmt_scheduled = $conn->prepare(
    "SELECT COUNT(DISTINCT user_id) as total 
     ...
     WHERE ? BETWEEN start_date AND end_date AND schedule_id > 0"
);
$stmt_scheduled->bind_param("s", $today);
...
foreach ($trend_data as $date_key => $data) {
    ...
    // (*** แก้ไขเป็น ***)
    // หายอดคนที่ "มีกะ" ใน "วันนั้นๆ" (N+1 query, แต่จำเป็นเพื่อความถูกต้อง)
    $stmt_scheduled_daily = $conn->prepare(
        "SELECT COUNT(DISTINCT user_id) as total 
         FROM employee_schedules 
         WHERE ? BETWEEN start_date AND end_date AND schedule_id > 0"
    );
    $stmt_scheduled_daily->bind_param("s", $date_key); // $date_key คือ YYYY-MM-DD
