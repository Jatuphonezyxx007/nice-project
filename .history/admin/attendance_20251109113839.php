...
// 5. (*** อัปเดต: SQL Query ***)
// (*** แก้ไข ***) ลบ u.schedule_id และ s.name (ตัวเก่า) ออกจาก SELECT
...
/* ... (ส่วนต้นของ SQL) ... */
        ...
        -- (*** โค้ดใหม่ - ถูกต้องแล้ว ***)
LEFT JOIN employee_schedules es ON u.id = es.user_id 
    AND a.attendance_date = es.shift_date -- (*** แก้ไขบรรทัดนี้ ***)
...
// (ส่วนที่เหลือของ PHP ด้านบน ... เหมือนเดิม)
                    ...
                    <tbody>
                        <?php if ($attendance_result->num_rows > 0): ?>
                            <?php while ($row = $attendance_result->fetch_assoc()): ?>
                                ...
                                <?php
                                // (*** ส่วนนี้ถูกต้องแล้ว ***)
                                // ตรรกะคำนวณสถานะ และ OT ทำงานกับ $row['time_in'], $row['time_out']
                                // ซึ่งดึงมาจากตารางกะที่ถูกต้องแล้ว
                                $status_text = 'N/A';
                                $status_bg = 'secondary';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // (*** คงไว้ ***) 1. ควบคุม Details Modal
    var detailsModal = document.getElementById('detailsModal');
    detailsModal.addEventListener('show.bs.modal', function (event) {
        // ... (โค้ดส่วนนี้ - เหมือนเดิม) ...
      ...
      detailsModal.querySelector('#detailsDate').textContent = 'วันที่: ' + formattedDate; 
    ...
    // (*** ลบส่วนนี้ ***) 2. ควบคุม Edit Shift Modal
    // (*** ลบส่วนนี้ ***) 3. จัดการการ Submit ของ Form (AJAX)

    // (*** คงไว้ ***) 4. ควบคุม Dropdown ผู้อนุมัติ (เปลี่ยนเลขเป็น 2)
