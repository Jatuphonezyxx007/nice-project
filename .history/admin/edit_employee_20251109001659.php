...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // (รับค่า)
    $id = (int)$_POST['id'];
    $employee_id = $_POST['employee_id']; // <-- ใช้ตัวนี้ตั้งชื่อ
    // ...
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == 0) {
        ...
        // 2. สร้างชื่อไฟล์ใหม่จาก รหัสพนักงาน (ที่อาจจะถูกแก้ไข)
        $profile_filename = $employee_id . '.' . $file_extension;
        $target_file = $target_dir . $profile_filename;

        // 3. ย้ายไฟล์ใหม่
        if (move_uploaded_file($_FILES["profile_image"]["tmp_name"], $target_file)) {
            // 4. ถ้าอัปโหลดสำเร็จ และชื่อไฟล์ใหม่ไม่ตรงกับชื่อเก่า ให้ลบไฟล์เก่า
            if (!empty($old_profile_image) && $old_profile_image != $profile_filename && file_exists($target_dir . $old_profile_image)) {
                unlink($target_dir . $old_profile_image);
            }
        } else {
            // อัปโหลดล้มเหลว, กลับไปใช้รูปเดิม
            $profile_filename = $old_profile_image;
        }
    }

    // (อัปเดต) ตรรกะอัปเดต (เมื่อมีรหัสผ่านใหม่)
    if (!empty($new_password)) {
        $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
        
        $sql = "UPDATE users SET 
                    prefix_th = ?, name_th = ?, lastname_th = ?, 
                    prefix_en = ?, name_en = ?, lastname_en = ?,
                    employee_id = ?, email = ?, position_id = ?, schedule_id = ?, 
                    profile_image = ?, password = ?
                WHERE id = ?";
        
        $stmt_update = $conn->prepare($sql);
        // (อัปเดต) bind_param (13 ตัวแปร)
        $stmt_update->bind_param(
            "ssssssssiissi", // 6x(s), 2x(s), 2x(i), 2x(s), 1x(i)
            $prefix_th, $name_th, $lastname_th, 
            $prefix_en, $name_en, $lastname_en,
            $employee_id, $email, $position_id, $schedule_id, 
            $profile_filename, $hashed_password, $id
        );
        
    } else {
        // (อัปเดต) ตรรกะอัปเดต (เมื่อไม่มีรหัสผ่านใหม่)
        $sql = "UPDATE users SET 
                    prefix_th = ?, name_th = ?, lastname_th = ?, 
                    prefix_en = ?, name_en = ?, lastname_en = ?,
                    employee_id = ?, email = ?, position_id = ?, schedule_id = ?, 
        ...
        $stmt_update->bind_param(
            "ssssssssiisi", // 6x(s), 2x(s), 2x(i), 1x(s), 1x(i)
            $prefix_th, $name_th, $lastname_th, 
            $prefix_en, $name_en, $lastname_en,
            $employee_id, $email, $position_id, $schedule_id, 
            $profile_filename, $id
    ...
    if ($stmt_update->execute()) {
      if ($id == $_SESSION['user_id']) {
            $_SESSION['name'] = $prefix_th . ' ' . $name_th . ' ' . $lastname_th;
            $_SESSION['profile_image'] = $profile_filename;
        }
...
// ดึงข้อมูล "ตำแหน่ง" และ "กะ" สำหรับ Dropdowns
        ...
        <div class="col-md-10">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-header">
                    แก้ไขข้อมูลสำหรับ: <?php echo htmlspecialchars($employee['prefix_th'].' '.$employee['name_th'] . ' ' . $employee['lastname_th']); ?>
                </div>
                <div class="card-body">
                    <form action="edit_employee.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5 mb-3">
                                ...
                                <input type="text" name="name_th" class="form-control" value="<?php echo htmlspecialchars($employee['name_th']); ?>" required>
                            </div>
                            <div class="col-md-5 mb-3">
                                <label class="form-label">นามสกุล (ไทย) <span class="text-danger">*</span></label>
                                <input type="text" name="lastname_th" class="form-control" value="<?php echo htmlspecialchars($employee['lastname_th']); ?>" required>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-2 mb-3">
                                <label class="form-label">Prefix (EN) <span class="text-danger">*</span></label>
                                <select name="prefix_en" class="form-control" required>
                                    <option value="Mr." <?php echo ($employee['prefix_en'] == 'Mr.') ? 'selected' : ''; ?>>Mr.</option>
                                    <option value="Mrs." <?php echo ($employee['prefix_en'] == 'Mrs.') ? 'selected' : ''; ?>>Mrs.</option>
                                    <option value="Miss" <?php echo ($employee['prefix_en'] == 'Miss') ? 'selected' : ''; ?>>Miss</option>
                                </select>
                            </div>
                            <div class="col-md-5 mb-3">
                                <label class="form-label">Name (EN) <span class="text-danger">*</span></label>
                                <input type="text" name="name_en" class="form-control" value="<?php echo htmlspecialchars($employee['name_en']); ?>" required>
                            </div>
                            <div class="col-md-5 mb-3">
                                <label class="form-label">Lastname (EN) <span class="text-danger">*</span></label>
                                <input type="text" name="lastname_en" class="form-control" value="<?php echo htmlspecialchars($employee['lastname_en']); ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="schedule_id" class="form-control" required>
                                    ...
                                    <?php 
                                    $schedules_result->data_seek(0); // Reset pointer
