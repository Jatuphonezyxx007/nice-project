...
// (*** ใหม่ ***) 4. ดึง Admin ทั้งหมด (สำหรับ Dropdown ผู้อนุมัติ)
...
// (*** อัปเดต ***) 5. ดึงข้อมูลการลงเวลาทั้งหมด (เพิ่ม time_in, time_out, approver_id)
$sql = "SELECT a.id as attendance_id, a.attendance_date, a.check_in_time, a.check_in_image, 
               ...
               s.time_in,  /* (ใหม่) เวลากะ (เข้า) */
               s.time_out  /* (ใหม่) เวลากะ (ออก) */
        FROM attendance a
                ...
                <table class="table table-striped table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>เวลาเข้า</th>
                            <th>ภาพ (เข้า)</th>
                            <th>สถานะ</th> <th>เวลาออก</th>
                            <th>ภาพ (ออก)</th>
                            <th>OT (ชม.)</th> <th>ผู้อนุมัติ</th> <th>จัดการ</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ($attendance_result->num_rows > 0): ?>
                            <?php while ($row = $attendance_result->fetch_assoc()): ?>
                                ...
                                <?php
                                // (*** ใหม่: ตรรกะคำนวณสถานะ และ OT ***)
                                
                                // 1. คำนวณสถานะ (Early, On Time, Late)
                                $status_text = 'N/A';
                                $status_bg = 'secondary';
                                if (!empty($row['check_in_time']) && !empty($row['time_in'])) {
                                    $check_in_timestamp = strtotime($row['check_in_time']);
                                    // สร้าง timestamp ของเวลากำหนดเข้างานใน "วันนั้น"
                                ...
                                // 2. คำนวณ OT (ต้องออกหลังเวลากะ 1 ชม. ขึ้นไป)
                                $ot_hours = 0;
                                if (!empty($row['check_out_time']) && !empty($row['time_out'])) {
                                    ...
                                    // ต้องเกิน 1 ชั่วโมง (3600 วินาที)
                                    if ($diff_seconds > 3600) { 
                                        $ot_hours = floor($diff_seconds / 3600); // ปัดลง
                                    }
                                }
                                // (*** จบส่วนตรรกะ ***)
                                ?>
                    ...
                    <button type="button" class="btn btn-sm btn-warning btn-edit-shift" data-bs-toggle="modal"
                      ...
                      <i class="bi bi-clock-history"></i>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="9" class="text-center text-muted">ไม่พบข้อมูลการลงเวลาในวันที่เลือก</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- 1. ควบคุม Details Modal (เหมือนเดิม) ---
    var detailsModal = document.getElementById('detailsModal');
    detailsModal.addEventListener('show.bs.modal', function(event) {
        // ... (โค้ดเดิม) ...
    });

    // --- 2. ควบคุม Edit Shift Modal (เหมือนเดิม) ---
